 ...
 */
    ...
    $(document).ready(function(){
        ...
        $( ".pagAjax" ).each(function() {
            var table=$(this).html();
            var form ='<form id="pagAjax_form" class ="form-inline" onsubmit="pagAjaxCargaDatos($(this));return false;"><div style="float: left;"><input type="hidden" id="pagAjax_order" name="order" /><input type="hidden" id="pagAjax_ot" name="ot" value="0" />Mostrar <select name="tampag" class="form-control" onchange="$(this).closest(\'#pagAjax_form\').submit()"> <option value="10">10</option><option value="20">20</option><option value="50">50</option></select>registros por página</div><div style="float: right;">Búsqueda: <input type="text" name="filtro" class="form-control"  onkeyup="$(this).closest(\'#pagAjax_form\').submit()"/> </div></form>'; 
            var code = '<div id="pagAjax_controles">'+form+'</div><div id="pagAjax_data" style="padding-top: 20px; margin-top: 20px;">'+table+'</div>';
            $(this).html(code);
            thOrder($(this)); 
...
//                $('.pagAjax').animate({ scrollTop: 1000 }, "slow");
                $('.pagAjax #pagAjax_data').html('<div class="load" style="color:#03A678"><i class="fa fa-5x fa-spinner fa-pulse"></i></div>');
                $('.pagAjax #pagAjax_data').load($(this).attr('href')+"&"+$('.pagAjax #pagAjax_form').serialize()+dataGet,function() {                    
                    thOrder($(this).parents().find(".pagAjax").find("#pagAjax_form"));
                });
...
function pagAjaxCargaDatos(obj){
    ...
    obj.closest('.pagAjax').find('#pagAjax_data').html('<div class="load" style="color:#03A678"><i class="fa fa-5x fa-spinner fa-pulse"></i></div>');
    ...
    obj.find('th').each(function() {
       ...
       if($(this).data().order){
           ...
           if(order_actual == order){
               if(order_ot == 0)
                    icono = '<i style="color:white;" class="fa fa-sort-amount-asc faa-tada animated-hover"></i>';
                else
                    icono = '<i style="color:white;" class="fa fa-sort-amount-desc faa-tada animated-hover"></i>';
           }else{
               icono = '<i style="color:white;" class="fa fa-sort faa-tada animated-hover"></i>';
           }
